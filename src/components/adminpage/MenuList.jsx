...
import { collection, getDocs, deleteDoc, setDoc, addDoc, doc } from "firebase/firestore";
...
const MenuList = () => {
  ...
  const handleEditSubmit = async event => {
    ...
    if (currentItem && currentItem.id) {
      try {
        await setDoc(doc(db, "menuItems", currentItem.id), updatedItem, { merge: true });
        setItems(prevItems => prevItems.map(item => item.id === currentItem.id ? { ...item, ...updatedItem } : item));
        setIsModalOpen(false);
      } catch (error) {
  ...
  return (
    <div className="space-y-4">
      ...
      {items.filter(item => item.EnDescription.toLowerCase().includes(searchQuery.toLowerCase()) ||
                            item.ChnDescription.toLowerCase().includes(searchQuery.toLowerCase()) ||
                            item.ItemID.toString().includes(searchQuery)).map(item => (
        <div key={item.id} className="p-4 mb-2 w-full bg-gray-100 rounded-lg shadow flex justify-between items-center">
          ...
          <div>
            <button onClick={() => handleEdit(item)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 mr-2 rounded-lg">Edit</button>
            <button onClick={() => handleDelete(item.id)} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg">Delete</button>
          </div>
        </div>
      ))}
      ...
      {items.length === 0 && <p className="text-center text-gray-500">No menu items found.</p>}
    </div>
  );
};
